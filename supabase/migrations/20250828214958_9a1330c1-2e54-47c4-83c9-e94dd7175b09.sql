...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Eras are readable by everyone'
  ) THEN
    CREATE POLICY "Eras are readable by everyone"
    ON public.eras FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE policyname = 'Knowledge items are readable by everyone'
  ) THEN
