  ...
  return (
    <div className={`${isMobile ? 'h-screen overflow-hidden' : 'h-screen overflow-hidden'} bg-background relative`}>
      ...
      <div className={`relative z-10 max-w-4xl mx-auto ${isMobile ? 'p-1 h-full overflow-y-auto scale-85 w-11/12 max-w-sm ml-2' : 'p-6'}`}>
        ...
        {/* Pergunta */}
        <div className={`arena-card-epic backdrop-blur-sm bg-cyan-500/10 border border-cyan-500 ${isMobile ? 'p-2 mb-2 mx-1 scale-75' : 'p-2 mb-2 mt-10 border-2 glow-epic scale-56'}`} style={isMobile ? {marginTop: '25%', width: '95%', marginLeft: 'auto', marginRight: 'auto'} : {}}>
          ...
          <h2 className={`font-montserrat font-bold text-center text-foreground ${isMobile ? 'text-sm mb-1' : 'text-2xl mb-8'}`}>
            {question.question}
          </h2>
            ...
            {question.options.map((option, index) => (
              ...
              >
                <div className={`flex items-center ${isMobile ? 'space-x-2' : 'space-x-4'}`}>
                  <div className={`rounded-full flex items-center justify-center font-bold ${isMobile ? 'w-6 h-6 text-sm' : 'w-10 h-10 text-lg'} ${
                  ...
                  <span className={`font-semibold ${isMobile ? 'text-sm' : 'text-lg'}`}>{option}</span>
                </div>
              </button>
            ))}
          </div>
